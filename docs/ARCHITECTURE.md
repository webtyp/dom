...
- `OnUserActivity(handler func())`: Registers the presence event set (`pointermove`, `pointerdown`, `keydown`, `wheel`, `scroll`) on `document` in **capture phase**, so the handler is called when the user does anything anywhere in the page.
  This exists because presence is a document-level fact, not an element-level fact. Registering presence listeners on a component's root element fails in three ways: `mouseenter` fires only once upon crossing the boundary and does not repeat with movement, `keydown` only arrives if focus is inside that subtree, and elements mounted outside the subtree (portals, overlays, toasts) emit no events to it. All three failure modes disappear with capture-phase document listeners.
  The handler receives no arguments (the only fact is that activity occurred) and is throttled to at most ~1 call/second. `dom` owns no timer, threshold, or concept of "idle". Register once; there is no way to unregister it. (Backend: no-op.)
