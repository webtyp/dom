...
| 9 | **El montaje reactivo no es síncrono.** Un pase de `BindChildren` que **crea** una fila no se ve en el DOM al volver de `Set()`: corre un microtask después. Es el precio de permitir un `Init()` bloqueante (`caller.Call` + `<-ch`) sin deadlockear el bucle de eventos WASM cuando el `Set()` viene de un callback asíncrono. | Se difiere **solo** cuando el pase crea filas, y **el pase entero** (inserciones y barrido de eliminación juntos, o un nodo reemplazado quedaría junto a su reemplazo). Mover/eliminar filas ya montadas sigue siendo síncrono. Pases encolados para el mismo padre mantienen orden FIFO. | ✅ `tests/uc_reactive_mount_deadlock_test.go`, `tests/uc_reactive_replace_test.go` |
